...
| `kind` | enum `SaleLineKind` | PRODUCT / FOOD / SERVICE, **snapshot, ไม่มี default** | ประเภทบรรทัด ณ เวลาขาย (20e) — แยกรายงาน อาหาร / นวดสปา / สินค้าหน้าร้าน · ทุกทางออกบิล (POS · ปิดบิลโต๊ะ/ห้อง · กลับบ้าน) ต้องระบุเอง · ลบเมนู/เปลี่ยนชนิดทีหลังแล้วรายงานย้อนหลังไม่เพี้ยน |
...
| `kind` | enum `TABLE | ROOM` | default TABLE | (Phase 20) ห้องนวด = จองล่วงหน้าได้ (20b) · ผังโต๊ะแยกกลุ่ม · ร้านนวดมีทั้งสองแบบร่วมกัน |
| `stationId` | String? | optional, FK → KitchenStation (SetNull) | (Phase 20) ประเภทห้อง (ห้องนวดเท้า/ห้องนวดไทย) — ตารางเดียวกับประเภทบริการของโปรแกรม เพื่อให้ "ห้องต้องตรงกับรายการนวด" เช็คได้ |
| `createdAt` | DateTime | auto | เวลาสร้าง |
| `updatedAt` | DateTime | auto | เวลาแก้ไขล่าสุด |
...
| `customerLabel` | String? | optional (2026-09-23) | ชื่อลูกค้าของบิล — **ห้องสปา (`Table.kind = ROOM`) เปิดได้หลาย session พร้อมกัน 1 ใบต่อลูกค้า** · โต๊ะอาหารยังมี session เปิดได้ครั้งละ 1 เหมือนเดิม |
...
| `itemType` | enum `FOOD | SERVICE` | default FOOD | (Phase 20) SERVICE = โปรแกรมนวด — ไม่เข้าครัว/KDS · ต้องมีพนักงานนวดตอนขายจากจอพนักงาน |
| `durationMinutes` | Int? | บังคับเมื่อ SERVICE (5–600) | (Phase 20) ระยะเวลาบริการ — ใช้คำนวณช่วงจอง/เวลาห้องไม่ว่าง (20b) |
| `createdAt` | DateTime | auto | เวลาสร้าง |
| `updatedAt` | DateTime | auto | เวลาแก้ไขล่าสุด |
...
#### Therapist *(Phase 20 — พนักงานนวด)*
| ฟิลด์ | ชนิด | เงื่อนไข | คำอธิบาย |
|---|---|---|---|
| `id` | String | PK | รหัสภายในระบบ |
| `storeId` | String | FK → Store (Cascade) | ของร้าน |
| `code` | String | required, unique ต่อร้าน, ≤ 12 | รหัสที่ลูกค้า/พนักงานเรียก เช่น "001" (เก็บเป็นตัวพิมพ์ใหญ่) |
| `name` / `nickname` | String / String? | — | ชื่อจริง / ชื่อเล่น (ป้ายที่แสดง = `code + nickname ?? name`) |
| `phone` / `gender` / `startedAt` / `note` / `imageUrl` | optional | — | ประวัติ · รูปเก็บที่ `StoreAsset` เดิม |
| `isActive` | Boolean | default true | ปิดใช้งาน = ไม่ขึ้นให้เลือกบนจอขาย/มอบหมาย · **ลบได้เฉพาะคนที่ไม่มีชื่อในบิล/ออร์เดอร์** (`SaleItem.therapistId` RESTRICT) |
| `userId` | String? | optional | ผูกบัญชีล็อกอินทีหลังได้ (ยังไม่ใช้) |
| `skills` | KitchenStation[] | m:n `_TherapistSkills` | ทักษะ = ประเภทบริการที่ทำได้ — จอขายกรองเฉพาะคนที่ตรงกับ `MenuItem.stationId` ของโปรแกรม (โปรแกรมไม่ระบุประเภท = ทุกคน) |
> **ไม่ผูกกับบัญชีผู้ใช้** — พนักงานนวดไม่ต้องล็อกอิน · ทักษะ/ห้อง/โปรแกรม ใช้ตาราง `KitchenStation` (Phase 19) ตัวเดียวกันเป็น "ประเภทบริการ" ·
> `MobileOrderItem.therapistId` (SetNull) = พนักงานของบรรทัดบริการ (ลูกค้าสั่งเองผ่าน QR เป็น null จนพนักงาน "มอบหมาย") ·
> `SaleItem.therapistId` (RESTRICT) = snapshot ลงบิลไว้ทำรายงาน/ค่ามือต่อคน (20c)

#### TherapistShift *(Phase 20b — กะทำงานของพนักงานนวด)*
| ฟิลด์ | ชนิด | เงื่อนไข | คำอธิบาย |
|---|---|---|---|
| `id` | String | PK | รหัสภายในระบบ |
| `storeId` | String | FK → Store (Cascade) | ของร้าน |
| `therapistId` | String | FK → Therapist (Cascade) | — |
| `workDate` | DateTime | `@db.Date`, unique ร่วมกับ `therapistId` | วันทำงานตามวันทางธุรกิจ (เวลาไทย) |
| `startMinute` / `endMinute` | Int | นาทีนับจากเที่ยงคืน (08:30 = 510) | เก็บเป็นนาทีเพื่อเทียบช่วงได้โดยไม่ยุ่งกับ timezone |
| `isOff` | Boolean | default false | วันหยุดของคนนี้ — จองคิวให้ไม่ได้ |
| `note` | String? | optional | — |
> เก็บ **รายวัน** ไม่ใช่รูปแบบประจำสัปดาห์ (ร้านนวดสลับกะ/ลาหยุดบ่อยจนรูปแบบประจำไม่เคยตรง) — ปุ่ม "คัดลอกไปทั้งสัปดาห์"
> เขียนแถวรายวันให้ครบจริง ๆ ตารางที่เห็นบนจอจึงตรงกับฐานเป๊ะ ๆ · **ไม่มีแถว = "ยังไม่ตั้งกะ" = จองคิว/เช็กอินไม่ได้** (เปลี่ยน 2026-09-24 · 20e — เดิมจองได้ · ขายหน้าร้าน/มอบหมาย walk-in ยังไม่ต้องมีกะ) · กระดานขึ้นว่ายังไม่ตั้งกะ
> ต่างจาก `isOff` ที่แปลว่าหยุดจริง

#### Booking *(Phase 20b — การจองล่วงหน้า)*
| ฟิลด์ | ชนิด | เงื่อนไข | คำอธิบาย |
|---|---|---|---|
| `id` | String | PK | รหัสภายในระบบ |
| `storeId` | String | FK → Store (Cascade) | ของร้าน |
| `customerName` / `customerPhone` | String / String? | ชื่อบังคับ | ลูกค้าไม่ต้องเป็นสมาชิก |
| `menuItemId` | String | FK → MenuItem (Restrict) | โปรแกรมนวด (`itemType = SERVICE`) |
| `durationMinutes` | Int | required | **snapshot ณ ตอนจอง** — แก้เวลาโปรแกรมในเมนูภายหลังต้องไม่ทำให้คิวที่จองไว้เลื่อน |
| `therapistId` | String | FK → Therapist (Restrict) | **บังคับตั้งแต่ตอนจอง** (ลูกค้ามักขอชื่อคนเดิม) |
| `tableId` | String? | FK → Table (SetNull) | ห้องนวด — เลือกทีหลังตอนเช็กอินได้ |
| `startAt` / `endAt` | DateTime | `endAt = startAt + durationMinutes` | เก็บ `endAt` ซ้ำเพื่อให้ query ช่วงทับซ้อนใช้ index เดียว |
| `status` | enum `BookingStatus` | default BOOKED | BOOKED → CHECKED_IN → IN_SERVICE → DONE · CANCELLED / NO_SHOW |
| `tableSessionId` | String? | FK → TableSession (SetNull) | session ที่เกิดตอนเช็กอิน |
| `checkedInAt` / `cancelledAt` / `cancelReason` / `note` / `createdById` | optional | — | ประวัติการทำรายการ |
> **ไม่ใช่บิล** — เป็นการกันเวลาของพนักงาน (และห้อง ถ้าเลือกไว้) เท่านั้น เงินเกิดตอนเช็กอิน ซึ่งเปิด `TableSession` + `MobileOrder`
> ด้วยเส้นทางเดิมแล้วปิดบิลที่หน้าเดิมทุกประการ (กติกาข้อ 8) · **กันคิวชนกันด้วย advisory lock ต่อพนักงาน (720_003) และต่อห้อง (720_004)**
> ใน `lib/booking.ts` ไม่ใช่ EXCLUDE constraint (Prisma ไม่รู้จัก จะ drift ทุก migrate diff) · สถานะ CANCELLED/NO_SHOW/DONE คืนช่วงเวลาให้จองใหม่ได้ทันที
...
| `spaEnabled` | Boolean | default false | (Phase 20) ตัวเลือกร้านนวด — เปิดแล้ว "เพิ่ม" พนักงานนวด/โปรแกรมนวด/ห้องนวด/จอง ของเดิมยังอยู่ครบ ใช้ร่วมกันได้ |
| `bookingBufferMinutes` | Int | default 10 | (Phase 20b) ช่วงพักระหว่างคิวของพนักงาน/ห้องเดียวกัน (นาที) — 0 = จองต่อกันติดได้ · ใช้ทั้งตอนตรวจคิวชนและตอนเสนอช่วงว่างในฟอร์ม |
...
- [x] รายการอาหารแสดง badge สถานะต่อรายการ (รอครัวรับ/กำลังปรุง/เสิร์ฟแล้ว) + ตัวปรับจำนวน
      > **ตัวปรับจำนวน — ตัดสินใจ 2026-09-17**: **ลดจำนวน**ได้เฉพาะรายการที่ครัวยังไม่รับ (`AWAITING_KITCHEN`) เท่านั้น
      > ของที่ครัวเริ่มทำแล้วต้องคิดเงิน · ทำแบบ ledger — `reduceOrderItemQuantity` ยกเลิกแถวเดิม (บันทึก `cancelledById` +
      > `cancelReason = "ลดจำนวนจาก N เป็น M"`) แล้วสร้างแถวใหม่ตามจำนวนใหม่ในทรานแซคชันเดียว ด้วย conditional update
      > ตัวเดียวกับยกเลิกรายการ (กติกาข้อ 7 · มีเทส concurrent กับครัวกดเริ่มทำ) · สิทธิ์ `MO_TABLES:DELETE` เท่ากับปุ่มยกเลิก
      > · **เพิ่มจำนวน** = ปุ่ม "สั่งเพิ่ม" พาไปจอขาย (`/mobile-order/pos?table=<id>`) เป็นรอบใหม่ → ครัวได้ทิกเก็ตใหม่
      > ไม่มี action แก้จำนวนขึ้น · ลดเหลือ 0 = ใช้ยกเลิกรายการ (ต้องมีเหตุผล)
...
- [x] **โหมดเริ่มต้นของจอขายอาหาร** (`StoreSettings.posDefaultMode` — 2026-09-17): ขายเข้าโต๊ะ / กลับบ้าน — หน้า
      `/mobile-order/pos` เปิดในโหมดนี้ทุกครั้ง (ยกเว้นเข้าจาก "สั่งเพิ่ม" ของโต๊ะที่เป็นโหมดโต๊ะเสมอ) · เจ้าของร้านตั้งที่ตั้งค่าร้าน
...
### F27 — ตัวเลือกร้านนวด: พนักงานนวด + โปรแกรมนวด + ห้องนวด (Phase 20a)
> เจ้าของสั่ง 2026-09-22: ระบบต้องใช้กับร้านนวดได้ **โดยใช้ร่วมกับของเดิม** (ร้านนวดมีทั้งโปรแกรมนวดและอาหาร/เครื่องดื่ม ทั้งห้องนวดและโต๊ะ) —
> จึงเป็น "สวิตช์เปิดเพิ่ม" (`StoreSettings.spaEnabled`) ไม่ใช่โหมดสลับ · ร้านอาหารเดิมไม่เห็นความเปลี่ยนแปลงใด ๆ

- [x] `StoreSettings.spaEnabled` (สวิตช์ในตั้งค่าร้าน OWNER + ติ๊กได้ตอน onboarding) — เปิดแล้ว "เพิ่ม" กลุ่มเมนู "ร้านนวด" ในแถบข้าง · ตัวเลือกชนิดเมนู/ชนิดโต๊ะในฟอร์ม
      · ปิดแล้วข้อมูลไม่หาย แค่ซ่อนเมนู
- [x] **`Therapist`** ต่อร้าน (§2) — หน้า `/spa/therapists` CRUD + รูป + ทักษะ + เปิด/ปิดใช้งาน + ป้าย "ว่าง/กำลังนวด" (จากบรรทัดบริการสถานะ COOKING ใน session ที่ยังเปิด)
      · resource **`SPA_THERAPISTS`** (+ `SPA_BOOKINGS` จองไว้ให้ 20b) · migration backfill ให้บทบาทเดิม (ตาม MO_MENU / MO_TABLES)
- [x] **`MenuItem.itemType = FOOD | SERVICE`** + `durationMinutes` (บังคับเมื่อ SERVICE · 5–600 นาที) — ฟอร์มเมนูมีปุ่ม "อาหาร / โปรแกรมนวด" (โผล่เมื่อ `spaEnabled`)
      · ประเภทครัวของ Phase 19 ใช้เป็น "ประเภทบริการ" ของโปรแกรม · หน้าลูกค้าแสดง "· 60 นาที"
- [x] **`Table.kind = TABLE | ROOM`** + `stationId` (ประเภทห้อง — ตาราง KitchenStation เดียวกัน · **FK จากฟอร์มเช็คว่าเป็นของร้าน**) — จัดการโต๊ะเลือก "โต๊ะอาหาร / ห้องนวด" ·
      ผังโต๊ะแยกกลุ่ม "ห้องนวด" / "โต๊ะ" เมื่อร้านมีห้อง · QR/เปิด session/ปิดบิล ใช้เส้นทางเดิมทั้งหมด
- [x] **ขายโปรแกรมนวดจากจอขาย** — บรรทัด SERVICE **ต้องเลือกพนักงานนวด** (`buildOrderLines({ requireTherapistForService })` · ปฏิเสธพนักงานของร้านอื่น/ปิดใช้งาน/ทักษะไม่ตรง ·
      อาหารระบุพนักงานไม่ได้) · dialog เลือกพนักงานกรองตามทักษะ + ป้าย "(ไม่ว่าง)" · `MobileOrderItem.therapistId` ถูกเขียนตอนสั่ง · ลูกค้าสั่งโปรแกรมผ่าน QR ได้โดยยังไม่มีพนักงาน
- [x] **โปรแกรมนวดไม่เข้าครัว** — `listKitchenTickets`/`getKitchenTicket`/ทิกเก็ต ESC-POS กรองเฉพาะ `FOOD` · หน้าห้อง (table-detail) มีปุ่ม **เริ่มนวด** (`startServiceItem`
      AWAITING_KITCHEN→COOKING ต้องมีพนักงานก่อน) และ **นวดเสร็จแล้ว** (`markItemServed` ข้าม READY ได้แม้ร้านเปิด KDS) + select **มอบหมาย/เปลี่ยนพนักงาน** (`assignOrderItemTherapist`
      conditional update เฉพาะ AWAITING_KITCHEN/COOKING · ทักษะต้องตรง) · สิทธิ์ `MO_TABLES:EDIT` หรือ `SPA_BOOKINGS:EDIT`
- [x] **ปิดบิลเส้นทางเดิม** — `SaleItem.therapistId` snapshot ตอน `closeSessionWithPayment` · ใบเสร็จ/ประวัติแสดง "· พนง. 001 นิด" · ไม่มีตารางบิลใหม่ (กติกาข้อ 8)
- [x] onboarding ติ๊ก "ร้านนวด/สปา" → `spaEnabled` + (เมื่อใส่ตัวอย่าง) ประเภทบริการ นวดไทย/นวดเท้า · โปรแกรม 3 · ห้อง 3/1, 5/1 · พนักงาน 001, 002
...
> · ตัดสินใจ 2026-09-23: กะเก็บ**รายวัน**พร้อมปุ่มคัดลอกทั้งสัปดาห์ · **พนักงานบังคับตั้งแต่ตอนจอง ห้องเลือกทีหลังตอนเช็กอินได้**

- [x] **`TherapistShift`** (§2) — หน้า `/spa/shifts` ตารางสัปดาห์ (แถว = พนักงาน · คอลัมน์ = 7 วัน) ตั้งเวลาเข้า–ออก/วันหยุด/ล้างกะ
      + ปุ่ม "คัดลอกไปทั้งสัปดาห์" ต่อวัน (เขียนทับวันปลายทางทั้งวัน ไม่ใช่รวมกัน) · สิทธิ์ `SPA_THERAPISTS:EDIT` (กะเป็นข้อมูลของพนักงาน)
- [x] **`Booking`** (§2) — หน้า `/spa/bookings` ตารางรายวัน `?date=YYYY-MM-DD` (จองล่วงหน้าได้ จึงยอมวันอนาคต — `parseDayKey` ไม่ใช่ `parseBusinessDayKey`)
      · กดช่องว่างของพนักงานเพื่อจองเวลานั้น · ฟอร์มกรองพนักงานตามทักษะและติดป้าย "(คิวชน)/(นอกกะ)" · ตารางรายการทั้งวันด้านล่าง
- [x] **กันจองซ้อนแบบ concurrent** — `assertSlotFree()` ใน `lib/booking.ts` จับ `pg_advisory_xact_lock(720_003, hashtext(therapistId))`
      แล้ว `(720_004, hashtext(tableId))` **ตามลำดับคงที่เสมอ** (สลับลำดับ = deadlock) ในทรานแซคชันเดียวกับการเขียน
      · ชนกันเมื่อช่วงเวลาซ้อนกันหลังขยายด้วย `bookingBufferMinutes` ทั้งสองข้าง · เทสยิงพร้อมกัน 10 คำขอ ผ่าน 1
- [x] **ความเข้ากัน** — โปรแกรมต้องเป็น SERVICE ที่ตั้งระยะเวลาแล้ว · พนักงานต้องมีทักษะตรง `MenuItem.stationId` · ห้องต้อง `kind = ROOM`
      ไม่ถูกรวม และประเภทห้องต้องตรงกับประเภทบริการ (ห้องไม่ระบุประเภท = ใช้ได้ทุกโปรแกรม) — **FK จากฟอร์มเช็คเองทุกตัว** (กติกาข้อ 5)
- [x] **กะคุมการจอง** — มีแถวกะแล้ว: หยุด/นอกเวลากะ จองไม่ได้ · **ไม่มีแถว = จองได้** (ร้านที่ยังไม่เริ่มตั้งกะต้องใช้งานได้ตามปกติ)
- [x] **เช็กอิน** (`checkInBooking`) — `SPA_BOOKINGS:ADD` + **`requireSellingStore()`** (ขายใหม่ · กติกาข้อ 5) → conditional update
      `status: BOOKED` ก่อนทำอย่างอื่น (กดพร้อมกันได้ออร์เดอร์ใบเดียว · เทส 5 คำขอพร้อมกัน) → `openOrReuseSession` + `buildOrderLines`
      ตัวเดียวกับจอขาย → `MobileOrder` ปกติ → **ปิดบิลเส้นทางเดิมทั้งหมด**
- [x] **สถานะเดินตามงานจริง** — "เริ่มนวด" (`startServiceItem`) → IN_SERVICE · "นวดเสร็จ" (`markItemServed`) → DONE ·
      ปิดบิล (`closeSessionWithPayment`) ปิดคิวที่ยังค้างของ session นั้นเป็น DONE ในทรานแซคชันเดียวกัน (กระดานไม่ค้าง)
- [x] **`/spa/board` กระดานสด** — พนักงานแต่ละคน: กำลังนวด (ห้อง + ถึงกี่โมง) / ว่าง / ยังไม่เข้ากะ / เลิกกะแล้ว / หยุด / ยังไม่ตั้งกะ
      + คิวถัดไป · การ์ดห้องนวด (ใครใช้อยู่ถึงกี่โมง · คิวถัดไป) · **คำนวณสดทั้งหมด ไม่มีตารางสถานะให้ค้าง** · SSE topic `bookings` + polling สำรอง
...
### ร้านนวด (Phase 20 — โผล่เมื่อ `StoreSettings.spaEnabled`)
- `/spa/therapists` — พนักงานนวด: ประวัติ/ทักษะ/เปิด-ปิดใช้งาน (F27 · สิทธิ์ `SPA_THERAPISTS`)
...
- **`lib/realtime.ts`** — event bus ในโปรเซสต่อร้าน (`publishStoreEvent(storeId, topic)` · topic: tables/orders/notifications/payments/menu/bookings)
...
#### ✅ 15a — ก พร้อมเพย์ตรงต่อร้าน (ขึ้น production แล้ว 2026-09-16, PR #6)
...
>    · **mock** ใช้พัฒนา/เทสโดยไม่เสียโควตา: รับ payload `MOCK|transRef|ยอด|เลขผู้รับ|นาทีก่อน` (transRef ขึ้นต้น DOWN = ล่ม, DUP = ซ้ำ)
>    ปฏิเสธสลิปจริงทุกใบ · **ห้ามตั้ง mock บน production**
> 2. **key เป็นของแพลตฟอร์ม โควตารวมทุกร้าน** รวมในค่าใช้งานรายวัน (~0.3 บาท/บิล เล็กเทียบ 10–60 บาท/วัน) — ไม่ให้ร้านกรอก key เอง
> 3. เบราว์เซอร์ลูกค้าอ่าน mini-QR บนสลิปเองด้วย `jsqr` (dynamic import · ย่อรูป ≤ 1600px · ลองกลับสี) ส่งเฉพาะ payload — **ไม่อัปโหลดรูป**
>    (`components/customer/slip-upload.tsx` โผล่ใต้ QR บน `pay/promptpay` เฉพาะร้านโหมด `PROMPTPAY_SLIP` ที่ตั้ง provider แล้ว)
> 4. **ด่าน 4 ชั้นใน `lib/slip-settle.ts`** (`verifySlipAndSettle`): ① ผู้รับ = บัญชีร้าน (`maskedAccountMatches` — ผู้ให้บริการปิดบางหลัก
>    เทียบจากท้าย ต้องเปิดเผย ≥ 4 หลัก · proxy↔พร้อมเพย์ / account↔เลขบัญชี ผ่านคู่ใดคู่หนึ่ง) ② ยอด ≥ บิลปัจจุบัน (เช็คก่อน + `verifiedAmount`
>    ใน `closeSessionWithPayment` เช็คซ้ำในทรานแซคชัน) ③ `Sale.paymentReference = "SLIP:<transRef>"` unique — เช็คก่อนเสียโควตา ·
>    สลิปที่ปิดโต๊ะอื่นแล้วใช้ซ้ำไม่ได้ · ส่งซ้ำโต๊ะเดิม/หลังปิดแล้ว = idempotent ④ เก่ากว่า `SLIP_MAX_AGE_MINUTES` (30) ไม่รับ
> 5. ไม่ผ่าน → **ไม่ปิดบิล**: ยอดขาด / ผ่านแต่ปิดบิลไม่ได้ → `Notification CHECK_BILL` ให้พนักงานพร้อมตัวเลข+transRef · ผู้ให้บริการล่ม →
>    บอกลูกค้าให้แจ้งพนักงาน (ตกไปโหมด ก) · ไม่มีการโพล/ลองใหม่เอง
> 6. เจ้าของร้านเลือกโหมด ก+ ได้เองที่ตั้งค่าร้านเมื่อแพลตฟอร์มตั้ง `SLIP_PROVIDER` แล้ว (ผู้ดูแลก็ตั้งให้ได้) · โหมดนี้ต้องมีเลขพร้อมเพย์
> 7. เทส `payment-slip.test.ts` 10 (สำเร็จ/โอนเกิน/ซ้ำ+ข้ามโต๊ะ/ผู้รับผิด/ปิดบางหลัก/ยอดขาด→แจ้ง/เก่า/ล่ม+ขยะ/โหมดไม่ใช่/แยกร้าน) + unit
>    `slip-qr.test.ts` 6 · **ตอน deploy**: ไม่ต้องมี env — ไม่ตั้ง = โหมด ก+ ปิด ระบบเหมือนเดิมทุกประการ · ไม่มี migration
- [x] **ก+ — พร้อมเพย์ตรง + ตรวจสลิปอัตโนมัติ**: หน้า `pay/promptpay` เพิ่ม "แนบสลิป" → อ่าน QR บนสลิป**ฝั่ง
...
### ✅ Phase 17 — พนักงานกดขายอาหารเองได้ (มีโต๊ะ / กลับบ้าน) + รูปภาพที่ร้านอัปโหลดเอง — ขึ้น production แล้ว 2026-09-17 (PR #16)
...
- [x] **กันจองซ้อนแบบ concurrent** — `lib/booking.ts`: `pg_advisory_xact_lock(720_003, hashtext(therapistId))` แล้ว `(720_004, hashtext(tableId))` **ลำดับคงที่** ·
      `resolveBookingTarget()` ตรวจโปรแกรม/ทักษะ/ประเภทห้อง · `assertWithinShift()` (ไม่มีแถวกะ = จองได้ → **20e เปลี่ยนเป็นจองไม่ได้**) · เทส 10 คำขอพร้อมกันผ่าน 1 · เช็กอินพร้อมกัน 5 ได้ออร์เดอร์ใบเดียว
- [x] `/spa/bookings` ตารางรายวัน `?date=` + ฟอร์ม (กรองทักษะ · ป้าย "คิวชน/นอกกะ" · ห้องไม่บังคับ) · `/spa/shifts` ตารางกะรายสัปดาห์ `?week=` ·
...
#### 20c — รายงานต่อพนักงานนวด (F29) — ✅ โค้ดเสร็จ · รวมเข้า PR ของ 20e (PR #28 เดิมถูก merge เข้า branch นี้)
...
#### 20d — ปรับหน้าจอร้านสปาหลังเจ้าของทดลองใช้ (เจ้าของสั่ง 2026-09-23 · 7 ข้อ)
- [x] (1)(4)(6) ร้านที่เปิด `spaEnabled` เห็นชื่อหน้า/เมนู "จัดการเมนูอาหาร/ร้านสปา" · "ขายอาหาร/ร้านสปา" · "ผังโต๊ะอาหาร/ห้องสปา"
      (`NavItem.spaLabel` ใน sidebar · `lib/spa-title.ts` ชื่อแท็บเบราว์เซอร์) · ร้านอาหารล้วนเห็นชื่อเดิม
- [x] (2)(5)(6) แท็บแยก อาหาร / นวดสปา (หน้าจัดการเมนู + จอขาย) และ โต๊ะอาหาร / ห้องสปา (ผังโต๊ะ) — `components/segment-tabs.tsx` ตัวเดียว
      · จอขายใช้ตะกร้าร่วมกันทุกแท็บ (บิลเดียวมีทั้งอาหารและนวดได้) · เพิ่มรายการจากแท็บไหนได้ชนิดของแท็บนั้นเป็นค่าเริ่มต้น
- [x] (3) "จอง 17:31 ไม่ได้" ไม่ใช่เรื่องเศษนาที — เป็น**พักระหว่างคิว 10 นาที** (`bookingBufferMinutes`) ที่ไม่เคยมีที่ให้ตั้งและข้อความไม่บอก
      → ช่อง "พักระหว่างคิวนวด (นาที)" 0–120 ในตั้งค่าร้าน (โผล่เมื่อเปิดสปา · ไม่ส่งมา = ไม่แตะค่าเดิม) · `clashMessage()` ใน `lib/booking.ts`
      บอกเวลาเริ่มได้เร็วสุด/ต้องจบก่อน พร้อมจำนวนนาทีพัก · hint ใต้ช่องเวลาในฟอร์มจอง
- [x] (7) **1 ลูกค้า = 1 บิล ในห้องสปา** — ห้องเดียวเปิดได้หลาย session พร้อมกัน (**1 บิล = 1 session ยังเป็นกติกาเดิม** รายงาน/ปิดรอบ/QR ไม่ต้องแก้)
      · migration `20260923150000_add_table_session_customer_label` (คอลัมน์ nullable ล้วน)
      · `openOrReuseSession` รับ `sessionId` (เข้าบิลที่เลือก — ต้องเป็นบิลเปิดของห้องนี้) / `newCustomer` (เปิดบิลใหม่แยก เฉพาะ ROOM)
      · เช็กอินคิวจอง: ลูกค้าคนเดิม (ชื่อตรง + เบอร์ตรงถ้ามีทั้งคู่ · `findCustomerSession`) เข้าบิลเดิม · คนอื่น = บิลใหม่ป้ายชื่อลูกค้า
      · จอขาย: ห้องที่มีบิลอยู่**ต้องเลือก** "เพิ่มในบิลของ…" หรือ "ลูกค้าใหม่ (เปิดบิลแยก)" + ชื่อ — server ปฏิเสธถ้าไม่เลือก
      · ปิดบิล/ยกเลิก: คืนห้องเป็นว่างเฉพาะเมื่อไม่มีบิลเปิดเหลือ (`releaseTableIfIdle` + `SELECT … FOR UPDATE` แถวห้อง กันปิดพร้อมกันแล้วห้องค้าง)
      · QR ชั่วคราวถูกตัดเมื่อห้องว่างจริงเท่านั้น · รวมโต๊ะไม่ได้เมื่อห้องมีหลายบิล
      · ผังห้องแสดงบิลแยกต่อลูกค้าพร้อมปุ่มปิดบิลรายใบ · หน้าออร์เดอร์/ปิดบิลรับ `?session=` + ตัวสลับบิล (`components/bill-switcher.tsx`)
      · ⚠️ ข้อจำกัดที่ยอมรับ: ลูกค้าสแกน QR ของห้องที่มีหลายบิล = เข้าบิลล่าสุด (ลูกค้าสปาแทบไม่สแกนเอง)
      · เทส `spa-split-bill.test.ts` 9 (รวมปิดสองบิลพร้อมกัน · ปิดบิลสุดท้ายพร้อมเปิดบิลใหม่) · tenant-isolation +2

#### 20e — ปรับร้านสปารอบ 2 + แยกรายงานอาหาร/นวด (เจ้าของสั่ง 2026-09-24 · ข้อ 2–8) — ✅ ขึ้น production แล้ว 2026-09-24 (PR #32)
> PR เดียวรวม 20c (#28) + 20d ข้อ 7 (#31) + ของใหม่ · **มี migration 2 ไฟล์ที่ยังไม่ขึ้น production**
> (`20260923150000_add_table_session_customer_label` จาก #31 และ `20260924090000_add_sale_item_kind`) → ต้อง backup + ซ้อมบนสำเนาก่อน merge
- [x] (2) **ไม่มีกะ = จองไม่ได้** — `assertWithinShift()` ปฏิเสธเมื่อไม่มีแถวกะ + ตรวจซ้ำตอน `checkInBooking` (กะถูกลบหลังจอง = เช็กอินไม่ผ่าน ถอยทั้งทรานแซคชัน)
      · ฟอร์มจองปิดตัวเลือกพนักงานที่ไม่มีกะ/หยุด · แถวบนไทม์ไลน์จาง + "ยังไม่ลงกะ · จองไม่ได้" · กดช่องว่าง = toast พร้อมปุ่มไปตารางกะ
      · **ไม่แตะ** จอขาย/มอบหมายพนักงาน walk-in (ไม่ต้องมีกะ) · คิวเก่าที่จองไว้ก่อนเปลี่ยนกติกาไม่ถูกยกเลิกเอง แต่จะเช็กอินไม่ผ่านจนกว่าจะลงกะ
- [x] (3) แท่งคิวบนตารางจองสีตามสถานะ (จองไว้/เช็กอิน/กำลังนวด/เสร็จ — ใช้ token เดียวกับ chip · `.booking-bar` ใน globals.css) + คำอธิบายสี · คิวที่เสร็จยังโชว์แบบจาง
- [x] (4) แถบเวลามีขีดเต็มชั่วโมง (ตัวเลข) + ขีดครึ่งชั่วโมง (เส้นประ ":30") ลากลงทุกแถว · เส้น "ตอนนี้" สีแดง (นาทีคำนวณฝั่ง server) · ช่องจองละเอียดครึ่งชั่วโมง
- [x] (5) **ห้องรอเริ่มนวด** — `listServicesAwaitingStart()` (บรรทัด SERVICE `AWAITING_KITCHEN` ในบิลเปิด · คำนวณสด ไม่มีปุ่มรับทราบ · walk-in ก็ขึ้น)
      · แถบบนผังโต๊ะ (กดแล้วสลับแท็บห้อง + เลื่อนไปการ์ด) · การ์ดห้องมีกรอบเตือน + ปุ่ม **เริ่มนวด** (ใช้ `startServiceItem` เดิม · ยังไม่มอบหมาย = ปุ่มไปหน้าห้อง)
      · การ์ด "ห้องรอเริ่มนวด" ในหน้าแจ้งเตือน + นับใน badge (`getPendingNotificationCount`)
- [x] (6) ที่เจ้าของเจอบนเว็บจริง = #31 ยังไม่ deploy · ปิดช่องที่เหลือ: `/billing` ของห้องหลายบิลที่ไม่มี `?session=` ต้องเลือกลูกค้าก่อน (ไม่เดาบิลล่าสุด) ·
      ลูกค้าสแกน QR ห้องที่มีหลายบิล **จ่ายเองไม่ได้** (`sharedRoom` · `/pay` บอกให้จ่ายกับพนักงาน · `startCustomerPayment`/`submitPaymentSlip` ปฏิเสธซ้ำด้วย `hasMultipleOpenBills()`)
- [x] (7) รายงานสปา: ตาราง **พนักงาน × วัน** (ยอด/ครั้ง + รวมแถว/คอลัมน์) + **พนักงาน × โปรแกรม** (`getTherapistDailyMatrix`) · การ์ดยอดนวดทั้งร้านแยก "ไม่ระบุพนักงาน"
- [x] (8) **แยกรายงานอาหาร / นวดสปา / สินค้าหน้าร้าน** — `SaleItem.kind` (enum `SaleLineKind` snapshot · migration เติมค่าบิลเก่า: productId → PRODUCT ·
      มีพนักงานนวดหรือเมนู SERVICE → SERVICE · ที่เหลือ FOOD · แล้วตั้ง NOT NULL ไม่มี default)
      · `/reports` เลือกช่วงวัน (`?from=&to=` · เพดาน 366 วัน `MAX_REPORT_DAYS`) · การ์ด 3 ประเภท + แถว "ค่าบริการ − ส่วนลด" (ท้ายบิล ไม่กระจายเข้าประเภท — เจ้าของเลือก)
        → บวกกันเท่ายอดสุทธิพอดี (มีเทส) · กราฟแท่งซ้อนรายวัน (`components/sales-kind-chart.tsx` · สี `--chart-1..3` ผ่าน validator) + ตารางตัวเลข · ขายดีแยกประเภท · สัดส่วนวิธีชำระตามช่วงเดียวกัน
      · **CSV** `GET /api/reports/sales-csv?from=&to=&kind=` (`lib/sales-csv.ts` · BOM ให้ Excel อ่านไทย · กันสูตร Excel · สิทธิ์ `REPORTS:VIEW` / ไฟล์นวดรับ `SPA_THERAPISTS:VIEW` ด้วย · `no-store`)
      · ปิดรอบ (`/pos/closing`) และรายงานแบรนด์ **ยังไม่แยก** ในรอบนี้ (เจ้าของเลือกเฉพาะหน้ารายงาน + รายงานสปา)
- [x] เทส: `booking.test.ts` +2 (ไม่มีกะ · กะถูกลบก่อนเช็กอิน · รอเริ่มนวดขึ้น/หาย) · `spa-split-bill.test.ts` +1 · `sales-by-kind.test.ts` 5 · `day.test.ts` +1 ·
      tenant-isolation +7 query · helper `createFullDayShifts()` (เทสที่จองคิวต้องปูกะก่อน) · **704 ทั้งชุดผ่าน** · `pnpm build` ผ่าน
- [x] **deploy 2026-09-24 (PR #32 · CI run 35960037250)**: backup `posmobileorderdb-20260924-121414.dump` (อยู่ใน `D:\MJD_Backup`) → ซ้อมบนสำเนา: ขึ้น 2 migration → 31 ·
      `migrate diff` สะอาด · `sale_item` 29 แถว = FOOD ทั้งหมด ไม่มี NULL · บิล COMPLETED 20 ใบ ฿6,815.00 = รายการ + ค่าบริการ − ส่วนลด (diff 0) → merge → สลับ green → blue ·
      ตรวจฐานจริง: `_prisma_migrations` = 31 · `kind` FOOD 29 · `ls .next/server/app/api/reports` ในคอนเทนเนอร์ blue = `sales-csv` · PR #28/#31 ปิด (รวมเข้า #32)
- [x] เจ้าของทดสอบหน้าจริงแล้ว → เจอเรื่องต่อ 3 ข้อ ทำเป็น 20f

#### 20f — QR ตอนปิดบิล + ตัวกรองรายงานสปา + hover ตารางจอง (เจ้าของสั่ง 2026-09-24 รอบ 2) — โค้ดเสร็จ รอ deploy
> ไม่มี migration ไม่มี env ใหม่
- [x] (1) **ปิดบิลด้วยพร้อมเพย์ต้องแสดง QR ก่อนเสมอ** — เดิม (ตั้งแต่ Phase 10) หน้าปิดบิลพนักงานเลือกพร้อมเพย์แล้วกดยืนยัน = ปิดบิลทันทีไม่มี QR
      · `prepareStaffPromptPay` (`MO_TABLES:EDIT` · **ยอดคิดที่ server จากบิล** · ไม่ปิดบิล) ตามโหมดร้าน: SCB ที่ต่อได้ = QR ธนาคารพก ref1 ผ่าน `issuePaymentIntent` เดิม (กดซ้ำได้ใบเดิม · ธนาคารล่ม = ถอยไป QR ร้าน)
        → บิลปิดเองเมื่อ callback มา (หน้าจอฟัง SSE + โพลสำรอง 3 วิ ผ่าน `getStaffBillStatus` ที่อ่านอย่างเดียว **ไม่ยิงถามธนาคาร**) · ร้านอื่น = QR พร้อมเพย์ของร้าน + ปุ่ม "ได้รับเงินแล้ว — ปิดบิล"
      · ปุ่มปิดบิลแบบพร้อมเพย์กดได้เมื่อแสดง QR ของยอดปัจจุบันแล้วเท่านั้น (ยอดเปลี่ยน = เตือนสร้างใหม่) · โหมด SCB ยังมีปุ่มปิดเองสำรองกรณี callback ไม่มา
- [x] (2) `/spa/reports` แถบตัวกรอง (`components/spa-report-filter.tsx`): ช่วงวัน · พนักงานนวด · ประเภทบริการ — URL `?from=&to=&therapist=&type=` ·
      `SpaReportFilter` ใน `getTherapistSalesReport`/`getTherapistDailyMatrix`/`listSalesForExport` (`spaFilterSql()`) · CSV พาตัวกรองไปด้วย ·
      ⚠️ ประเภทบริการอ่านจาก `MenuItem.stationId` ปัจจุบัน (ไม่ snapshot) — ย้ายประเภทโปรแกรมทีหลัง รายงานย้อนหลังย้ายตาม
- [x] (3) ตารางจอง hover: แท่งคิว = ลูกค้า · โปรแกรม · **เริ่ม–สิ้นสุด** · พนักงาน/ห้อง · สถานะ · ช่องว่าง = "จองได้ HH:MM · ว่างถึง HH:MM" (คิวถัดไป − พัก หรือหมดกะ) / ไม่ว่าง / นอกกะ
...
- [ ] deploy (ไม่มี migration — merge → CI) · ตรวจ: ปิดบิลพร้อมเพย์เห็น QR จริง · ตัวกรองรายงาน · hover ตารางจอง
